...
ns.listNotebooks().then(function (books) {
    var p1 = books.map(function (book) {
                ...
                return Model.findOne({guid: model.guid}).exec(function(err,note) {
                    ...
                    if(!note){
                        ...
                        return newModel.save(function (err) {
                            if (err){
                                console.log("出错："+ model.name + "---------------------------------" + model.title + "\n");
                                return reject(err);
                            }
                            console.log('新创建：' + model.name + "-------------" + model.title );
                            return resolve(model);
                        });
                    }
                    if(model.updated == note.updated) {
                        console.log('未修改：' + model.name + "-------------" + model.title);
                        return resolve(model);
                    }
                    return note.update({$set: model}, function (err) {
                        if (err) return reject(err);
                        console.log('更新：' + model.name + "-------------" + model.title );
                        return resolve(model);
                    });
                });
            })
        ...
        var p3 = ns.listNotesByNoteBookGuid(book.guid).then(function (data) {
            var notes = data.notes;
            return mapSeries(notes, function (note) {
                return ns.getNoteByGuid(note.guid).then(function (note) {
                    return saveMongodbPromise(note,notesModel).then(function () {
                        console.log('updateNote: %s', note.title);
                    });
                });
            });
        });
        return RSVP.all([p2, p3]);
    });
    return RSVP.all(p1);
}).then(function () {
    console.log("\n 数据导入完毕!!!！");
